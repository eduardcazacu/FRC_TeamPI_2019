var map = function(p){
  ...
  p.setup = function() {
    ...
    //p.frameRate(1);
    redDwarf = new Robot(width/2, height/4, 0);
  }
  ...
  p.draw = function() {
    ...
    redDwarf.draw(Xvalue/zoom + width/2, Yvalue/zoom + height/2, Anglevalue);
    if(destination != null){
      destination.draw();
      //MoveredDwarf();
    }
  ...
  function MoveredDwarf(){
    ...
    switch(index){
      case 0: //calculate de veriables
        oldX = destination.x;
        oldY = destination.y;
        oldDir = (redDwarf.dir+360)%360;
        angleToGo = calcAngleToGo(destination.x,destination.y);
        distance = p.sqrt(p.pow(destination.x-redDwarf.x,2),p.pow(destination.y-redDwarf.y,2));

        index++;
        //no break because it needs to do this always just 1 time
      case 1://turn around
        if(redDwarf.turn(angleToGo)){
          index++;
        }
        break;
      case 2://move forward
        if(redDwarf.drive(distance)){
          oldDir = redDwarf.dir;
          index++;
        }
        break;
      case 3://turn to designated angle
        if(redDwarf.turn((destination.dir-oldDir+5*360)%360)){
          index++;
        }
        break;
    }
